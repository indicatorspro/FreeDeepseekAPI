'use strict';
/*
  Common DeepSeek auth parser from "Copy as cURL" or HAR file.
  Used by both CLI scripts (scripts/auth_from_*.js) and dashboard
  endpoint POST /api/accounts/import. Returns flat object
  { token, cookie, hif_dliq, hif_leim, wasmUrl } or { error }.
*/
...
// Optional \$? before the quote covers Chrome's "Copy as cURL (bash)" form.
...
function parseHar(harText) {
    ...
    catch { return { error: 'Failed to parse HAR (not valid JSON)' }; }
    const entries = (har.log && har.log.entries) || [];
    const hv = (hs, n) => { const x = (hs || []).find(y => (y.name || '').toLowerCase() === n); return x ? (x.value || '') : ''; };

    // pick best request to deepseek with Authorization: Bearer
    let best = null;
    ...
    if (!best) return { error: 'No requests to deepseek.com with Authorization: Bearer header in HAR' };
...
// Auto-detect input format (HAR — JSON with log.entries; otherwise cURL).
function parseAuthInput(text) {
    const s = String(text || '').trim();
    if (!s) return { error: 'Empty input' };
    if (s[0] === '{' || s[0] === '[') {
        // ready-made JSON {token,cookie,...} (e.g. from a browser extension export)
        try {
        ...
        } catch { /* not a plain JSON object — try HAR below */ }
        const r = parseHar(s);
        if (!r.error) return r; // it was a HAR
    }
    if (/\bcurl\b|--header|(^|\s)-H\s/i.test(s)) return parseCurl(s);
    // last attempt — maybe HAR without explicit prefix
    return parseHar(s);
}

// Validation + fill wasmUrl (from input → from previous account → default).
