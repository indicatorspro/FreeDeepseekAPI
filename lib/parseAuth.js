'use strict';
/*
  Shared DeepSeek auth parser from "Copy as cURL" or HAR file.
  Used by CLI scripts (scripts/auth_from_*.js) and the dashboard endpoint
  POST /api/accounts/import. Returns a flat object
...
// Optional \$? before quote covers "Copy as cURL (bash)" form.
...
function parseHar(harText) {
    ...
    catch { return { error: 'Failed to parse HAR (not JSON)' }; }
    const entries = (har.log && har.log.entries) || [];
    const hv = (hs, n) => { const x = (hs || []).find(y => (y.name || '').toLowerCase() === n); return x ? (x.value || '') : ''; };

    // pick the best request to deepseek with Authorization: Bearer
    let best = null;
    ...
    if (!best) return { error: 'No requests to deepseek.com with Authorization: Bearer header found in HAR' };
...
function parseAuthInput(text) {
    ...
    if (s[0] === '{' || s[0] === '[') {
        // ready JSON {token,cookie,...} (e.g., from extension exporter)
        try {
        ...
        } catch { /* not a JSON object — try HAR below */ }
        const r = parseHar(s);
        if (!r.error) return r; // it was HAR
    }
    if (/\bcurl\b|--header|(^|\s)-H\s/i.test(s)) return parseCurl(s);
    // last resort — maybe HAR without explicit prefix
    return parseHar(s);
}
